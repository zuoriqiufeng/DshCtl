...
> ⚠️ **版本锚点**：本篇 file:line 核实于 `0d1f50007f`（v0.1.6-alpha.1）；上游已升级至 `00102833df`（v0.1.7-alpha.2，2026-09-23），行号普遍漂移 10~25 行。本轮已逐行复核并更新工具管线与 defineTool 相关行号（均附旧值）；漂移对照表见 [dsh-plugin-reading-guide.md §5](../dsh-plugin-reading-guide.md#5-版本锚点与漂移警告)。

---

## 1. turn / step：Agent 的两级节奏

- **定义**（官方 `deepseek-harness/docs/architecture.md` turn flow 节）：
  - **step** = 一次模型请求（含它带出的工具调用）；
  - **turn** = 用户一轮输入到最终答复，由 **0..n 个 step** 组成（模型可能连续多轮「调工具 → 看结果 → 再调」）。
- **代码坐标**（v0.1.7-alpha.2）：`turn()` 开轮在 `deepseek-harness/packages/core/agent-loop/src/agent.ts:295`；每个 step 记 `step/start`（`:328`）→ 流式回包 → `step/end`（`:338`）。旧锚点为 `:270 / :303 / :313`。
...
- **模型只见三件套**：`name / description / parameters`（JSON-schema，`deepseek-harness/packages/llm/llm/src/types.ts:461` 的 `ToolSchema`；官方口径 `packages/core/tools/README.md`）。
- **`defineTool` 完整面**（`deepseek-harness/packages/core/tools/src/schema.ts:554`（v0.1.7-alpha.2；旧锚点 :545），选项类型 `DefineToolOptions` :483-536）：

  | 字段 | 干什么 |
  |---|---|
  | `name` / `description` | 模型可见的身份 |
  | `parameters` | 属性级 schema，编译成 JSON Schema 并生成 `validate(args)` |
  | `output { schema, render, presentationMeta? }` | 结果契约 + 渲染（**必填**，`index.ts:224-225`） |
  | `execute(args, exec)` | 工具体；**包装为「先 validate(args) 再进用户 execute」**——模型参数边界的校验铁律（AGENTS.md §7） |
  | `timeoutMs?` / `isConcurrencySafe?` | 外部 policy 使用；**timeoutMs 永不发给模型**（`index.ts:262-266`） |
  | `finalizeContent?` / `presentCall?` / `presentResult?` | 展示层钩子，软校验（过期回退默认值不抛） |

- **注册**：`ctx.tools.register(def)`（`packages/core/tools/src/index.ts:1062`；旧锚点 :1043）做二次结构校验（output 契约、run_code 保留名）后插入 scoped 分层表，**返回 disposer 即注销**。schema 自动喂进系统提示（`index.ts:853` 的 `ctx.systemPrompt.tools(...)`）。
...
prepareExecution()
  ...
  │     packages/core/tools/src/index.ts:1505-1508（`ctx.waterfall(carrier, 'tools/pre-execute', …)`）
  │     决策：allow / deny / cancel / ask（ask → 接用户审批 :1509-1512）
  ├─ ② 单调 guard（any guard 可 deny，no guard 可 force-allow）
  │     index.ts:1518（`guardReason(exec)`，实现 :766-772 / :1144-1150）
  └─ ③ 全过 → dispatch → tools/execute waterfall（工具体在内层跑）:1605
        → tools/post-execute :1781 → 冻结结果 tools/result :1703
```

> ⚠️ 上述行号为 v0.1.7-alpha.2 实测；v0.1.6-alpha.1 时为 `:1482-1486 / :1107-1124 / :1583 / :1741 / :1675`。按关键词（`'tools/pre-execute'`、`guardReason`、`'tools/execute'`）重新定位最稳。

- **阶段顺序不变量由测试强制**：pre 每个 execution 只触发一次、execute 必须跟在 pre 后、post 必须最后（`packages/core/tools/src/invariant.ts:95-111`）。
- **waterfall 默认行为**：内层 callback 是 `{ kind: 'allow' }`（`:1507`）——监听器不调 `next()` 就是否决（见 [03-cordis-concepts.md](03-cordis-concepts.md) §6）。
...
- **harness 唯一业务用法**：`AgentRegistry` 的 initiator 因果链——两个 ALS 存「当前异步链的发起 Agent」（`deepseek-harness/packages/core/agent/src/index.ts:248-249`），`withInitiator/withoutInitiator` 建立/清除因果边界（`:327, :342`）。
...
- 下一篇：[06-reading-guide.md](06-reading-guide.md)——176 篇官方文档怎么查、哪 5 个坑别踩。
