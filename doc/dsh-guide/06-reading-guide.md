...
**本篇你会学到**：harness 官方 176 篇 docs 的分层地图（怎么查）；本工作区文档分工再收束；5 条最容易误解的点（误解 → 真相 → 出处）；读完的 5 问自测。

> 想按「插件机制」主题把官方文档串成一条阅读路线，见 [dsh-plugin-reading-guide.md](../dsh-plugin-reading-guide.md)——它另外讲清了官方两条教学线（Cordis 教程线 / Harness 产品线）对 `cordis.yml` 说法相反的原因。

---

## 1. 官方 docs 地图（`deepseek-harness/docs/`，176 篇英文，171 篇配 `.zh.md`）

```
入门/学 ── docs/architecture.md          总纲：Cordis / profiles+bundles / launch / turn flow
          docs/cordis-primer.md          框架精华 51 行（五想法+五模式+waterfall+loader+规则）
          docs/cordis-tutorial/          8 篇教程：index + first-plugin → lifecycle → services →
                                          events → config → composition-and-hmr → into-the-harness
查（子系统）docs/subsystems/*.md         63 篇，一页一子系统（`subsystems/README.md` 与总纲互补）：
...
写插件     docs/user/develop/*.md        10 篇：basic 4 / framework 3 / practice 3（产品向落地路径）
参考       docs/cordis-api/*.md          6 篇 context / fiber / service / registry / events（生成式 API）
...
血泪       docs/postmortem/*.md          5 起事故复盘
规范       docs/AGENTS.md                文档标准："one home per fact"（:15）——写新文档先读
```

**查法口诀**：概念 primer → 做事 cookbook → 某子系统不熟 subsystems → 改代码前 postmortem。**不要在新文档里重新定义术语**——一个事实只在官方 docs 有一个家（`docs/AGENTS.md:15` 的 tier taxonomy 表）。

## 2. 本工作区文档分工（收束）

| 想干什么 | 看哪 |
|---|---|
| **读** 官方插件文档（路线图） | `doc/dsh-plugin-reading-guide.md` |
...
- **出处**：`deepseek-harness/vendor/cordis/src/reflect.ts:80-91`（特殊属性放行）、`:144`（无 inject 报错）、`:153`（`internal/get` waterfall）、`:160`（inactive context 报错）；规则 `packages/AGENTS.md:6`（Optional services 条：可选依赖用 `ctx.get`）。
...
- **出处**：`vendor/include/src/index.ts:105`（id 必填）、`:120-123`（逐键赋值）；`packages/bundle/base/cordis.patch.yml:6`；`docs/architecture.zh.md:27`。
...
- **出处**：`vendor/cordis/src/events.ts:194`（emit）、`:228`（waterfall veto 注释）、`:234`（waterfall 实现）；`vendor/loader/src/index.ts:104`；`packages/core/tools/src/index.ts:1505`；`packages/core/tools/src/invariant.ts:95-111`。
...
- **出处**：`vendor/cordis/src/fiber.ts:612-623`；`packages/bundle/base/cordis.patch.yml:12`（"Row order carries no load semantics"）；教程 `docs/cordis-tutorial/02-lifecycle-and-effects.zh.md:82`（"why does my plugin print nothing?"）。
...
- **出处**：`vendor/loader/src/config/entry.ts:74`（`get disabled()`）、`:89`（`disabledOf()`）；`vendor/loader/src/index.ts:104`；`vendor/include/src/index.ts:162-167`；`vendor/README.md` 修改日志。
